...
W_fc2 = weight_variable([240, 120])
b_fc2 = bias_variable([120])
W_fc3 = weight_variable([120, 2])
...
if args.train:
    te_dd=pickle.load(f)[:,:101]
    te_bb=pickle.load(f)[:,101:]
    fig_x=[]
    fig_y=[]
    for i in range (2500):  #training times
        try:
            ...
            if i<1000:
                ...
                sess.run(train_step, feed_dict={xs: dd, ys:y_d, keep_prob: 0.5, lr:0.00001})
                sess.run(train_step, feed_dict={xs: bb, ys:y_b, keep_prob: 0.5, lr:0.00001})
            if i % 5 == 0:
                print(i)
    ...
    for i in range (samples*100):
        ...
        for j in range(100):
            if predict_b[j][0]>=predict_b[j][1]:
                error_cnt_b+=1
    error_rate_d=(float)(error_cnt_d/(samples*10000.0))
    error_rate_b=(float)(error_cnt_b/(samples*10000.0))
    accuracy_rate=1-(float)(error_cnt_b+error_cnt_d)/(samples*20000.0)
