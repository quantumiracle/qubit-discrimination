...
W_fc1 = weight_variable([num_bins, 1])
b_fc1 = bias_variable([1])


saver = tf.train.Saver()  #define saver of the check point

prediction = tf.nn.relu(tf.matmul(tf.reshape(xs,[-1,num_bins]), W_fc1) + b_fc1)
...
if args.test:
    ...
    for k in range (test_samples):
        for i in range (100):
            ...
            predict_d = sess.run(prediction, feed_dict={xs: dd, keep_prob: 1})
            for j in range(100):
                if predict_d[j][0]<0.5:   #darck state should >0.5 (1)
                    error_cnt_d+=1
            predict_b = sess.run(prediction, feed_dict={xs: bb, keep_prob: 1})
            for j in range(100):
                if predict_b[j][0]>=0.5:  #bright state should <0.5 (0)
                    error_cnt_b+=1
