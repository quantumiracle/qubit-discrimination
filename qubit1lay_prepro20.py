...
import math
import gzip
save_file='./model1lay_prepro20.ckpt'
...
fig = plt.figure()
f =gzip.open('./DetectionBinsData_pickle_prepro20.gzip','rb')  #10000*105 times measurement
if args.train:
    ...
    for i in range (2500):  #training times
        try:
            ...
            #without first row
            data=np.array(pickle.load(f))  #prepro is list data!!
            dd=data[:,:num_bins]
            y_d=100*[[1,0]]
            bb=data[:,num_bins:]
            y_b=100*[[0,1]]
...
if args.test:
    ...
    error_cnt_d=0
    saver.restore(sess, save_file)
    test_samples=2   #test_samples * 100 *100 = test nums
    ...
    accuracy_rate=1-(float)(error_cnt_b+error_cnt_d)/(test_samples*200*100.0)
