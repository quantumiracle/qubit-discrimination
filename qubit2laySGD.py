...
loss = tf.reduce_mean(tf.reduce_sum(abs(ys - prediction),
                                        reduction_indices=[1])) 
#loss = tf.reduce_sum(abs(ys - prediction))

#train_step = tf.train.AdamOptimizer(lr).minimize(loss)
#train_step = tf.train.GradientDescentOptimizer(lr).minimize(loss)
train_step = tf.train.MomentumOptimizer(lr, momentum=0.9).minimize(loss)
sess = tf.Session()
# important step
...
if args.train:
    ...
    for i in range (2500):  #training times
        try:
            ...
            if i<1000:
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: dd, ys:y_d, keep_prob: 0.5, lr:0.01})
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: bb, ys:y_b, keep_prob: 0.5, lr:0.01})
            elif i<2000:
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: dd, ys:y_d, keep_prob: 0.5, lr:0.0000001})
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: bb, ys:y_b, keep_prob: 0.5, lr:0.0000001})
            else:
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: dd, ys:y_d, keep_prob: 0.5, lr:0.00000001})
                _, curr_loss=sess.run([train_step,loss], feed_dict={xs: bb, ys:y_b, keep_prob: 0.5, lr:0.00000001})
            if i % 5 == 0:
                print(i)
                error=compute_accuracy(te_dd,y_d)
                #error+=compute_accuracy(te_bb,y_b)
                #print(error)
                curr_loss=curr_loss*100.0
                print(curr_loss)
                fig_x.append(i)
